...
const createSendToken= (user,statusCode,res)=>{
   ...
   const token= signInTotken(user._id)
...
exports.signUpUser= catchAsync(async(req,res,next)=>{
    ...
    const token= signInTotken(newUser._id)
  })

  exports.logIn= catchAsync( async (req,res,next)=>{
    const {email,password}=req.body
// check if email or password exists
    if(!email || !password) return next(new AppError('email or password can not be empty',400))
...
if(currentUser.changedPasswordAfter(decoded.iat)){
    return next(new AppError('user currently changed password. pls log in again',401))
}
...
const url=`${ req.protocol }://${ req.get('host') }/api/v2/users/resetPassword/${ resetToken }`;

const message= `forgot your password? submit a patch request with your new password and passwordConform to : ${url}.\n if you did not forget password, please ignore this email`;
